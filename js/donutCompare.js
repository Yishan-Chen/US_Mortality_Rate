function donutCompare1(year, state){
  ...
  var drawSvg = d3.select("#donutchartCompare1").append("svg")
  .attr("id", "donutCompareC1")
...
function donutCompare2(year, state){
  margin = {top: 80, right: 50, bottom: 100, left: 200};
  ...
  var drawSvg = d3.select("#donutchartCompare2").append("svg")
  .attr("id", "donutCompareC2")
  .attr("width", width + margin.right + margin.left)
  .attr("height", height + margin.top + margin.bottom)
  .append("g")
  .attr("transform", "translate(" + (width/2 + margin.left) + "," + (height/2 + margin.top) + ")");
