function RadarChart(year, state) {
  ...
  d3.csv(csvFile, function (error, csv) {
    if(error){
      ...
      data.push(data1)
/*
      var data2 = csv.filter(csv => csv.Year === year && csv.CauseName !== "All causes" && csv.State === "Texas")
      var total2 = 0;
      ...
      data2.forEach(function(d){
        d.percentage = +d.Deaths/total2;
      })
      data.push(data2)

      var data3 = csv.filter(csv => csv.Year === year && csv.CauseName !== "All causes" && csv.State === "Pennsylvania")
      var total3 = 0;
      //console.log(filter_data)
